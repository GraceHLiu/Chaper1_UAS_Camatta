...
#The error is because cumulated APAR of 4-30 can not be extracted, just ignore it for now
import os
...
#Extract pre-topocorrected values and post-topocorrected values
# Mother_Product_dir = 'C:/Users/Grace Liu/Box Sync/research/sUAS/JastroProposal'
# Product_dir = [os.path.join(Mother_Product_dir, 'II_Clipiped_Resampled/SR_30cm'), os.path.join(Mother_Product_dir, 'III_Topo_Correct/SR_30cm_CModel')]
# Products = ['rededge_b','red_b','nir_b','green_b','blue_b','rededge_a','red_a','nir_a','green_a','blue_a']
# After_bands = ['rededge','red','nir','green','blue']
# ShapeFile_dir = 'C:/Users/Grace Liu/Box Sync/research/sUAS/JastroProposal/Spatial_data'
# Field_list = ['comment','DSM_111120','Slope_1111','Aspect_111','AspectC_11','FlowA_1111']
# OutPut_dir = 'C:/Users/Grace Liu/Box Sync/research/sUAS/JastroProposal/III_Topo_Correct/SR_30cm_CModel/Comparison'
#
...
#             print 'extracting ' + product + ' for shapefile ' + shapefile
#             if len(os.listdir(product))==6:
#                 raster_dir = glob.glob(product + '/Camatta_' + date_str + '.tif')[0]
#                 print 'opening raster (without topo-correction) ' + os.path.basename(raster_dir)
#                 # https://gis.stackexchange.com/questions/46893/getting-pixel-value-of-gdal-raster-under-ogr-point-without-numpy
#                 src_ds = gdal.Open(raster_dir)  # open raster
#                 gt = src_ds.GetGeoTransform()
#                 for band in range(src_ds.RasterCount):
#                     band += 1
#                     print "[ GETTING BAND ]: ", band
#                     print "[ GETTING BAND ]: " + After_bands[band-1]
...
#                     intval = rb.ReadAsArray(px, py, 1,1)  # intval is a tuple, length=1 as we only asked for 1 pixel value
#                     Fields_onelyr.append(intval[0][0])
#                     print 'extracted value (before) of band '+ After_bands[band-1] + '= ' + str(intval[0][0])
#             else:
#                 for after_band in After_bands:
#                     raster_dir = glob.glob(product + '/Camatta_' + date_str + '_Topoed_'+ after_band +'.tif')[0]
#                     print 'opening raster (after topo-correction) ' + os.path.basename(raster_dir)
#                     # https://gis.stackexchange.com/questions/46893/getting-pixel-value-of-gdal-raster-under-ogr-point-without-numpy
#                     src_ds = gdal.Open(raster_dir)#open raster
#                     gt = src_ds.GetGeoTransform()
#                     rb = src_ds.GetRasterBand(1)
...
#                     intval = rb.ReadAsArray(px, py, 1, 1) # intval is a tuple, length=1 as we only asked for 1 pixel value
#                     Fields_onelyr.append(intval[0][0])
#                     print 'extracted value (after) of band '+ after_band + '= ' + str(intval[0][0])
#         Fields_alllyr.append(Fields_onelyr)
#         Fields_alllyr_df = pd.DataFrame(Fields_alllyr[1:],columns=Fields_alllyr[0])
#         Fields_alllyr_df.to_csv(os.path.join(OutPut_dir, 'ClippingAll_' + date_str + '_BATopo.csv'), index=False, header=True)
#     print 'finished extracting of file ' + shapefile
